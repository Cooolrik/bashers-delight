...
namespace bdr
	{
	Device::Device( const Instance* _module ) : MainSubmodule(_module) 
		{
		LogThis;
		}
	...
	status Device::Cleanup()
		...
		SafeVkDestroy( DeviceHandle , vkDestroyDevice( this->DeviceHandle, nullptr ) );
		SafeVkDestroy( SurfaceHandle , vkDestroySurfaceKHR( this->GetModule()->GetInstanceHandle(), this->SurfaceHandle, nullptr ) );

		return status_code::ok;
		}

	status_return<CommandPool*> Device::CreateCommandPool( const CommandPoolTemplate& parameters )
		{
		...
		CheckCall( vkCreateCommandPool( this->DeviceHandle, &poolInfo, nullptr, &commandPoolHandle ) );
		...
		CheckCall( vkAllocateCommandBuffers( this->DeviceHandle, &commandBufferAllocateInfo, bufferObjects.data() ) );

		// create a CommandPool object 
		auto commandPool = unique_ptr<CommandPool>( new CommandPool( this->GetModule(), commandPoolHandle , bufferObjects ) );
					
		// insert into map
		auto pCommandPool = commandPool.get();
		this->CommandPools.insert( { pCommandPool , std::move( commandPool ) } );

		// return the pool to the caller
		return pCommandPool;
		}

	status Device::DestroyCommandPool( CommandPool *commandPool )
		{
		auto it = this->CommandPools.find(commandPool);

		Validate( it != this->CommandPools.end() , status_code::invalid_param ) << "commandPool is not found in the list of allocated command pools" << ValidateEnd;
